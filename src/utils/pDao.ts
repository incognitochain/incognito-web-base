...
export const makeSignature = async (requestType: any, payload: any = {}, privateKey: any) => {
  let governanceHelper = new web3.eth.Contract(JSON.parse(GOVERNANCE_HELPER_ABI));
  let result: any = {};
  let signData;
...
export const genETHAccFromOTAKey = async (otaKey: any, payload: any) => {
  try {
    ...
    let index = await StorageManager.getItem(proposalID);
    if (!index) {
      index = 0;
      await StorageManager.setItem(proposalID, '0');
    } else {
      index = parseInt(index) + 1;
      await StorageManager.setItem(proposalID, index.toString());
    }
...
//   let signerWallet;
//   if (signerAddr) {
//     signerWallet = JSON.parse(await StorageManager.getItem(TEMP_WALLET_INFO));
